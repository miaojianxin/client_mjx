...
public:
											...
											ConsumeConcurrentlyContext& context)
	{
		...
		for (;it != msgs.end();it++)
		{
            ...
            printf("Topic[%s],Msg[%d:%d]: %s, strName:%s\n", 
                context.messageQueue.getTopic().c_str(), 
                consumeTimes, ++i, str.c_str(),strName.c_str());
		}
...
int main(int argc, char* argv[])
{
	...
	//mjx debug modify
	//DefaultMQPushConsumer consumer2("MyConsumerGroup");
	//consumer2.setNamesrvAddr(argv[1]);
	...
	//mjx debug modify
	char szPid2[20];
    sprintf(szPid2, "PID[%lu]_B", pid);

	//consumer2.setInstanceName(szPid2);
   // consumer2.setConsumeMessageBatchMaxSize(3);
	...
	*/
	...
	consumer.subscribe("CCR", "TagA || TagC || TagD || TagB || TagE");

	//mjx debug modify
	//consumer2.subscribe("CCR", "TagB || TagE");

    MessageListener *listener = NULL;

	//mjx debug modify

	MessageListener *listener2 = NULL;
	
    if(bOrderConsumer)
    {
        listener = new MsgOrderListener();

		//listener2 = new MsgOrderListener();
    }
    else
    {
        listener = new MsgListener("TagA || TagC || TagD || TagB || TagE");
		
		//listener2 = new MsgListener("TagB || TagE");
	...
	//consumer2.registerMessageListener(listener2);
